...
import { fetchAllNews } from "@/action/fetch";

export default async function page() {
  const fetchedNews = await fetchAllNews()
  // console.log(fetchedNews, "Finally");
  return (
    <div className="p-8">
        ...
        <TabsContent value="blogs" className="space-y-8">
          <div className="flex justify-between items-center border-b border-gray-200 dark:border-gray-600 py-3">
            <h2 className="scroll-m-20  text-2xl text-green-800 dark:text-white font-semibold tracking-tight first:mt-0">
              Recent Total Energies News ({fetchedNews.length})
            </h2>
          ...
          <div className="py-4">
            <DataTable data={fetchedNews} columns={columns} />
          </div>
        </TabsContent>
